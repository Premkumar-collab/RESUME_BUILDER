...
const EditResume = () => {
  ...
  const handleDeleteResume = async () => {
    try {
      setIsLoading(true);
      await axiosInstace.delete(API_PATHS.RESUME.DELETE_RESUME(id));
  ...
  const fetchResumeDetailsById = async () => {
    try {
      const response = await axiosInstace.get(API_PATHS.RESUME.GET_BY_ID(id));
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const uploadResponse = await axiosInstace.put(
  ...
  const updateResumeDetails = async (thumbnailLink) => {
    try {
      ...
      await axiosInstace.put(API_PATHS.RESUME.UPDATE_RESUME(id), {
        ...resumeData,
